...
class myPDF extends FPDF{
    function header1($db, $userId){
        ...
        $stmt7 = $db->query("select user_streetnumber from users WHERE user_id = '$userId'");
        $data7 = $stmt7->fetch(PDO::FETCH_OBJ);
        
        $this->SetFont('Arial','', 12);
        $this->Cell(30,2,'',0,1);
        $this->Cell(130,5,'Musterstrasse '.$data7->user_streetnumber,0,0);
        $this->Cell(59,5,'',0,1);
    ...
    function sum($db, $userId){
        ...
        $stmt4 = $db->query("select sum(betrag) as sumbetrag from rechnung WHERE fk_userId = '$userId' AND status = 'offen'");
